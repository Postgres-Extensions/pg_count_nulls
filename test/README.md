...
- `../bin/compare_fresh_vs_update` — not part of the pgTAP suite itself: a
  ...
  `bin/test_existing update-scenario`, which installs fresh and
  0.9.6-then-updated copies of the extension in their own scratch databases
  and diffs `pg_get_functiondef`/comments/ACLs for every object the
  extension owns. Catches an update script leaving some definition subtly
  different from a fresh install, even when the fixed pgTAP suite above
  still passes (it only asserts the specific behaviors it happens to check).
