...
`<extension>` builds on pgxntool (https://github.com/Postgres-Extensions/pgxntool);
...
release under) and the **extension version** (`<extension>.control`'s
`default_version`, what `CREATE EXTENSION <extension>;` installs by default and
...
(`sql/<extension>--<version>.sql`) still matches what that version actually
...
- [ ] For each `sql/<extension>--<version>.sql`, find its last-touching commit
...
- [ ] Confirm that commit is no later than when that EXTENSION version actually
      ...
      to cross-checking https://pgxn.org/dist/`<extension>`/, remembering that
      page lists *distribution* versions, which can lag the extension version
      they contain (see `0.9.7` above).
...
- [ ] **Default to committing every versioned install script.** For a small
      ...
      coverage isn't worth even the small cost. Update scripts
      (`sql/<extension>--<prev>--<version>.sql`) are ALWAYS committed
...
> `default_version` (in `<extension>.control`) normally sits at the literal string
> `'stable'`, so that ordinary source edits regenerate `sql/<extension>--stable.sql`
...
> number here points that same generation rule at `sql/<extension>--<version>.sql`
...
- [ ] If the extension version is moving, bump `default_version` in
      `<extension>.control` (bumped by hand). If only the distribution version is
...
- [ ] Bump the version in `META.in.json` — the source of truth is the top-level
      ...
      `provides.<extension>.version` (the extension version; only bump if it's
...
- [ ] If the extension version moved: add the update script
      `sql/<extension>--<prev>--<version>.sql`; confirm `ALTER EXTENSION <extension>
...
- [ ] From a clean checkout (or `git archive` of the tag): `make && make install`
      regenerates and installs cleanly and `CREATE EXTENSION <extension>;` reports the
...
- [ ] `make dist` — depends on `tag` (and builds the HTML docs), then
      `git archive`s the tag into `../<extension>-<version>.zip` (parent directory).
...
- [ ] Upload the `../<extension>-<version>.zip` to PGXN (manual).
...
- [ ] As soon as the release is merged, flip `default_version` back to `stable` in
      `<extension>.control`, open a new top `stable` section in `HISTORY.md`, and
      re-seed a fresh `sql/<extension>--<this-release>--stable.sql` update script
